...
class Deep_Neural_Network(tfg.Graph):
    def __init__(self, input_size, output_size, input_node, target_node, initializer, activator, optimizer,
                 learning_rate):
...
class Multi_Layer_Network(Deep_Neural_Network):
    ...
    def learning(self, max_epoch, data, batch_size=1000, print_period=10, is_numba=False, verbose=False):
        ...
        for fold_idx in range(data.n_splits):
            ...
            if fold_idx == 0:
                self.set_learning_process_specification(data, batch_size, 0, print_period, is_numba, fold_idx,
                                                        max_epoch, verbose)
            ...
            for epoch in range(1, max_epoch + 1):
                ...
                self.set_learning_process_specification(data, batch_size, epoch, print_period, is_numba, fold_idx,
                                                        max_epoch, verbose)
            ...
            self.test_accuracy_at_min_validation_error_epoch = float(
                self.test_accuracy_list[self.min_validation_error_epoch])
            ...
            print(
                "Global Epoch:{:3d} (Fold:{:3d} & Epoch:{:3d}) - Train Error:{:6.5f} - Validation Error:{:6.5f} - Test Accuracy:{:6.5f}".format(
    ...
    def set_learning_process_specification(self, data, batch_size, epoch, print_period, is_numba, fold_idx, max_epoch,
                                           verbose):
    ...
    def draw_output_description(self, figsize=(20, 5)):
        ...
        plt.subplot(245)
        for idx in range(self.hidden_layer_num):
            plt.plot(epoch_list, self.output_mean_list['activation'][idx], color_dic[idx],
                     label='activation' + str(idx))
        plt.ylabel('Mean')
        plt.xlabel('Epochs')
        plt.grid(True)
        plt.legend(loc='lower left')
        ...
        plt.subplot(246)
        for idx in range(self.hidden_layer_num):
            plt.plot(epoch_list, self.output_variance_list['activation'][idx], color_dic[idx],
                     label='activation' + str(idx))
        plt.ylabel('Variance')
        plt.xlabel('Epochs')
        plt.grid(True)
        ...
        plt.subplot(247)
        for idx in range(self.hidden_layer_num):
            plt.plot(epoch_list, self.output_skewness_list['activation'][idx], color_dic[idx],
                     label='activation' + str(idx))
        plt.ylabel('Skewness')
        plt.xlabel('Epochs')
        plt.grid(True)
        ...
        plt.subplot(248)
        for idx in range(self.hidden_layer_num):
            plt.plot(epoch_list, self.output_kurtosis_list['activation'][idx], color_dic[idx],
                     label='activation' + str(idx))
        plt.ylabel('Kurtosis')
        plt.xlabel('Epochs')
        plt.grid(True)
